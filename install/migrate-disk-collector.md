# RoachNet — About the Disk Collector Migration Script

This script migrates your RoachNet installation from the old host-based disk info collector to the new disk-collector sidecar. It modifies `/opt/project-nomad/compose.yml` to add the new service and remove the old bind mount, then restarts the full compose stack to apply the changes.
...
### Why does RoachNet need the nomad-disk-info.json file?
RoachNet uses the disk info stored and updated in `nomad-disk-info.json` to allow users to view disk usage and availability within the RoachNet Command Center. While not critical to the core functionality of RoachNet, it provides a more pleasant experience for users with limited storage space and/or who aren't familiar with command-line tools and Linux management.
